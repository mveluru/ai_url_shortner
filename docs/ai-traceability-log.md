...
- [edited] design v2.8, §22: Swagger URLs corrected to the implementation (canonical UI `/swagger-ui/index.html`, `/swagger-ui.html` redirects; contract `/v3/api-docs.yaml`; `/v3/api-docs` not served). The §22.3 snippet said `api-docs.path: /v3/api-docs`, which is wrong in practice (V-11) and would make `/v3/api-docs` answer; replaced with the implemented config and a URL table. Local cells measured live; only two prod cells measured, the rest labelled as config-derived.
