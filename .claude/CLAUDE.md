...
- **Swagger UI:** `http://localhost:<port>/swagger-ui/index.html` (`/swagger-ui.html` redirects to it); the contract it shows is `http://localhost:<port>/v3/api-docs.yaml`, served byte-identical from `docs/openapi.yaml` (`SwaggerIT`, design §22), never springdoc's generated spec (that one is hidden at `/v3/generated-api-docs`). Both exist only in `local`/`test`; `prod` returns 404. Use the port the run script printed, not a hard-coded 8080.
